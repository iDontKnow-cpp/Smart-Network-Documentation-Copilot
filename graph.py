...
def generate_response(state: GraphState):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a senior Systems Reliability Engineer. Use the provided context to answer the question clearly and directly. Do not output any plan, internal workflow, or reasoning steps. Return only the final answer in README.md style markdown. If the context does not contain enough evidence to answer, say 'I cannot answer this based on the retrieved data.'"),
        ("human", "Conversation history:\n{history}\n\nContext:\n{context}\n\nQuestion:\n{question}")
    ])
    
    fallback_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a senior Systems Reliability Engineer and an agentic RAG assistant. Internally compare local documentation and web search results, but do not expose any plan, evaluation, or evidence workflow in the final response. Output only the final answer in README-style markdown with this structure:\n\n# <Short title>\n\n## Introduction\n- One short sentence describing the task.\n\n## Conclusion\n- One concise sentence stating the final answer.\n\nDo not mention evidence sources, web/local, plans, scores, or internal reasoning. Keep the answer direct and factual."),
        ("human", "Conversation history:\n{history}\n\nLocal Context:\n{local_context}\n\nWeb Context:\n{web_context}\n\nQuestion:\n{question}")
    ])
    ...
    def clean_answer(answer: str):
        ...
        filtered = [line for line in lines if not re.match(r'^(Plan|Evidence|Evaluation|References|Source|Scoring|Local|Web|Conclusion):', line.strip(), re.IGNORECASE)]
        cleaned = '\n'.join(filtered).strip()
