...
* **Chat File Uploads:** `POST /api/upload` accepts PDFs and images. PDFs are classified into `docs/aws`, `docs/azure`, `docs/gcp`, `docs/nutanix`, `docs/vmware`, or `docs/other`, then immediately chunked and embedded into ChromaDB. Images are kept under `tmp/chats/<chat_id>` for the chat lifetime and are sent to the vision-capable chat model with the next question; abandoned chat files expire after 24 hours by default.
